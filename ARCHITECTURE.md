...
PC BIOS
  ...
  -> on Enter: raw journal record through BIOS INT 13h to LBA 128+
```
...
Stage two is one flat binary assembled through `%include`; source files remain modular without introducing an object linker. State is direct memory: integer cents, a transaction counter, an item count, and a small price stack. On transaction completion, `journal.asm` reads one 512-byte sector into `8000:0000`, changes one 32-byte record, writes the whole sector with BIOS `INT 13h`, and reads it back to validate the checksum. Debug text goes to QEMU's port 0xE9 independently of VGA.
